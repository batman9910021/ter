    ...
    if AUTO_DELETE_UPLOAD_MESSAGE_DURATION != -1:
        ...
        if message.chat.type == 'private':
            ...
            if EMOJI_THEME is True:
                warnmsg = f'<u>❗ This message will be deleted in <i>{auto_delete_message} minutes</i> from this group.</b>\n'
            else:
    ...
    if is_gdrive_link(link):
        ...
        if button in ["cancelled", ""]:
            ...
            if FORCE_BOT_PM is False:
                if PICS:
                    msg = sendPhoto(result + cc + warnmsg, bot, message, random.choice(PICS), button)
                else:
                    msg = sendMarkup(result + cc + warnmsg, bot, message, button)
