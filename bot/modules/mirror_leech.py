...
from bot.extraModules.extramodule import BuildSourceLinkButton, GetAutoDeleteMessage, checkModule
from bot import *
...
from bot.helper.telegram_helper.message_utils import sendMessage, sendMarkup, delete_all_messages, update_all_messages, auto_delete_upload_message, auto_delete_message, editMessage
...
def _mirror_leech(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False):
    ...
    if reply_to is not None:
        ...
        if len(link) == 0 or not is_url(link) and not is_magnet(link):
            if file_ is None:
                ...
                bxt = sendMessage("<b>Processing Your Telegram File...</b>", bot, message)
                Thread(target=TelegramDownloadHelper(listener).add_download, args=(message, f'{DOWNLOAD_DIR}{listener.uid}/', name)).start()
                editMessage(f"<b>{uname} Your Requested Telegram File Has Been Added To The Status</b>\n\n<b>Use /{BotCommands.StatusCommand} To Check Your Progress</b>\n", bxt)
                if multi > 1:
                    sleep(4)
    ...
    if is_gdrive_link(link):
        if not isZip and not extract and not isLeech:
            ...
            bxg = sendMessage("<b>Processing Your G-Drive Link...</b>", bot, message)
            Thread(target=add_gd_download, args=(link, f'{DOWNLOAD_DIR}{listener.uid}', listener, is_gdtot, is_unified, is_udrive, name)).start()
            if not isZip:
                editMessage(f"{uname} Your Requested G-Drive File Has Been Added To The Status For Extracting</b>\n\n<b>Use /{BotCommands.StatusCommand} To Check Your Progress</b>\n", bxg)
            else:
                editMessage(f"{uname} Your Requested G-Drive Folder Has Been Added To The Status For Archiving</b>\n\n<b>Use /{BotCommands.StatusCommand} To Check Your Progress</b>\n", bxg)
    elif is_mega_link(link):
        Thread(target=add_mega_download, args=(link, f'{DOWNLOAD_DIR}{listener.uid}/', listener, name)).start()
    elif isQbit and (is_magnet(link) or ospath.exists(link)):
        bxq = sendMessage("<b>Processing Your QBit...</b>", bot, message)
        Thread(target=QbDownloader(listener).add_qb_torrent, args=(link, f'{DOWNLOAD_DIR}{listener.uid}',
                                                                   ratio, seed_time)).start()
        if reply_to is not None:
            editMessage(f"<b>{uname} Your Requested QB Torrent File Has Been Added To The Status</b>\n\n<b>Use /{BotCommands.StatusCommand} To Check Your Progress</b>\n", bxq)
        elif link.startswith("magnet"):
            editMessage(f"<b>{uname} Your Requested QB Magnet Link Has Been Added To The Status</b>\n\n<b>Use /{BotCommands.StatusCommand} To Check Your Progress</b>\n", bxq)
    else:
        if len(mesg) > 1:
            ussr = mesg[1]
            if len(mesg) > 2:
            ...
            auth = ''
        bxa = sendMessage("<b>Processing Your Aria2c...</b>", bot, message)
        Thread(target=add_aria2c_download, args=(link, f'{DOWNLOAD_DIR}{listener.uid}', listener, name,
                                                 auth, ratio, seed_time)).start()
        if reply_to is not None:
            editMessage(f"<b>{uname} Your Requested Torrent File Has Been Added To The Status</b>\n\n<b>Use /{BotCommands.StatusCommand} To Check Your Progress</b>\n", bxa)
        elif link.startswith("magnet"):
            editMessage(f"<b>{uname} Your Requested Magnet Link Has Been Added To The Status</b>\n\n<b>Use /{BotCommands.StatusCommand} To Check Your Progress</b>\n", bxa)
        elif link.endswith(".torrent"):
            editMessage(f"<b>{uname} Your Requested Torrent Link Has Been Added To The Status</b>\n\n<b>Use /{BotCommands.StatusCommand} To Check Your Progress</b>\n", bxa)
        elif '0:/' in link or '1:/' in link or '2:/' in link or '3:/' in link or '4:/' in link or '5:/' in link or '6:/' in link or "workers.dev" in link:
            editMessage(f"<b>{uname} Your Requested Index Link Has Been Added To The Status</b>\n\n<b>Use /{BotCommands.StatusCommand} To Check Your Progress</b>\n", bxa)
        else:
            editMessage(f"<b>{uname} Your Requested DDL Has Been Added To The Status</b>\n\n<b>Use /{BotCommands.StatusCommand} To Check Your Progress</b>\n", bxa)
