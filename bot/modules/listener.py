...
from bot.extraModules.extramodule import BuildSourceLinkButton, GetAutoDeleteMessage
...
class MirrorLeechListener:
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name):
        buttons = ButtonMaker()
        pmwarn1 = f"<b>Your File(s) Are Successfully Mirrored.</b>\n\n"
        pmwarn2 = f"\n<b>Your File(s) Are Successfully Leeched.</b>\n\n"
        ...
        if AUTO_DELETE_UPLOAD_MESSAGE_DURATION != -1:
            warnmsg = GetAutoDeleteMessage(self.bot, self.message)
        else:
            warnmsg = ""
        ...
        if MIRROR_LOGS and self.message.chat.type != 'private':
            if EMOJI_THEME is True:
                ...
                logwarn = f"<b>Click Below Button To View Files.</b>"
        elif self.message.chat.type == 'private':
            logwarn = ''
        else:
            logwarn = ''
        if LEECH_LOG and self.message.chat.type != 'private':
            if EMOJI_THEME is True:
                ...
                logleechwarn = f"<b>Click Below Button To View Files.</b>"
        elif self.message.chat.type == 'private':
            logleechwarn = ''
        else:
            logleechwarn = ''
        ...
        if self.isLeech:
            ...
            if BOT_PM is True and FORCE_BOT_PM is False and self.message.chat.type != 'private':	
                ...
                botstart1 = f"https://t.me/+1SHciNENSI83M2Nl"	
                botstart = f"http://t.me/{b_uname}"	
                buttons.buildbutton("Get Your Files", f"{botstart}")
                buttons.buildbutton("Join Logs Channel", f"{botstart1}")
            elif self.message.chat.type == 'private':
                botstart = ''
            else:
                botstart = ''
            ...
            if EMOJI_THEME is True:
                ...
                # msg += f'\n<b>├ It Tooks:</b> {get_readable_time(time() - self.message.date.timestamp())}'
                msg += f'\n<b>Elapsed Time:</b> {get_readable_time(time() - self.message.date.timestamp())} | <b>#cc:</b> <code>{self.tag}</code>\n\n'
                ...
                for index, (link, name) in enumerate(files.items(), start=1):
                    ...
                    if len(fmsg.encode() + msg.encode()) > 2000:
                        ...
                        if FORCE_BOT_PM is False:
                            if PICS:
                                uploadmsg = sendPhoto(msg + fmsg + pmwarn2 + pmwarn + logleechwarn, self.bot, self.message, random.choice(PICS), buttons.build_menu(2))
                            else:
                                uploadmsg = sendMarkup(msg + fmsg + pmwarn2 + pmwarn + logleechwarn, self.bot, self.message, buttons.build_menu(2))
            ...
            if MIRROR_LOGS:	
                try:	
                    for chatid in MIRROR_LOGS:	
                        logmirresult = bot.sendMessage(chat_id=chatid, text=msg,	
                                       reply_markup=buttons.build_menu(2),	
                                       parse_mode=ParseMode.HTML)
                        logmirresult_link = logmirresult.link
                except Exception as e:	
                    LOGGER.warning(e)	

            botstart1 = f"https://t.me/+1SHciNENSI83M2Nl"	
            userbutton.buildbutton("Get Your Files", logmirresult_link) 
            userbutton.buildbutton("Join Logs Channel", f"{botstart1}")               
            if BOT_PM and self.message.chat.type != 'private':	
                try:	
            ...
            if FORCE_BOT_PM is False or self.message.chat.type == 'private':
                if PICS:
                    uploadmsg = sendPhoto(msg + pmwarn1 + pmwarn + logwarn, self.bot, self.message, random.choice(PICS), userbutton.build_menu(2))
                else:
                    uploadmsg = sendMarkup(msg, self.bot, self.message, userbutton.build_menu(2))
                Thread(target=auto_delete_upload_message, args=(bot, self.message, uploadmsg)).start()
